...
export async function kvZRangeByScore(setKey, min, max) {
  ...
  return kvFetch(`/zrangebyscore/${encodeURIComponent(setKey)}/${min}/${max}`);
}
...
// RPUSH key value -> /rpush/<key>/<value> — value should already be a string
// (callers JSON.stringify their own payloads, same convention as kvZAdd's member).
export async function kvRPush(key, value) {
  return kvFetch(`/rpush/${encodeURIComponent(key)}/${encodeURIComponent(value)}`, { method: "POST" });
}

export async function kvLRange(key, start, stop) {
  const res = await kvFetch(`/lrange/${encodeURIComponent(key)}/${start}/${stop}`);
  return Array.isArray(res) ? res.map((v) => maybeParseJson(v)) : [];
}

export async function kvSAdd(key, member) {
  return kvFetch(`/sadd/${encodeURIComponent(key)}/${encodeURIComponent(member)}`, { method: "POST" });
}

export async function kvSCard(key) {
  const res = await kvFetch(`/scard/${encodeURIComponent(key)}`);
  return Number(res) || 0;
}
