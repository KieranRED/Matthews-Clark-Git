...
export async function pcLeadsDetail() {
  ...
  return leads.map((lead) => {
    ...
    return {
      ...
      // Joins this lead back to its anonymous pre-contact trail — see
      // lib/pcTracking.js / app/api/pc/track. device/isWebview are the
      // snapshot taken at contact-capture (quiz-lead route), not looked up
      // live, so they match whatever the session's other events are tagged
      // with.
      sessionId: lead.sessionId || null,
      device: lead.device || null,
      isWebview: !!lead.isWebview,
