...
export const PAINT_CORRECTION_SERVICES = [
  { id: "pc_street_gloss", label: "Paint correction - Street Gloss", baseService: "correct", packageId: "stage-one", price: 5500, durationDays: 1 },
  { id: "pc_bronze", label: "Paint correction - Bronze", baseService: "correct", packageId: "bronze", price: 8500, durationDays: 1 },
  { id: "pc_silver", label: "Paint correction - Silver", baseService: "correct", packageId: "silver", price: 10500, durationDays: 2 },
  { id: "pc_gold", label: "Paint correction - Gold", baseService: "correct", packageId: "gold", price: 16000, durationDays: 3 },
  { id: "pc_diamond", label: "Paint correction - Diamond", baseService: "correct", packageId: "diamond", price: 30000, durationDays: 7 }
];

export const SERVICES = [
  { id: "detail", label: "Detail" },
  { id: "correct", label: "Paint correction" },
  ...PAINT_CORRECTION_SERVICES,
...
function customServiceLines(lead) {
  const customServices = Array.isArray(lead?.customServices) ? lead.customServices : [];
  const legacyCustomServices = (Array.isArray(lead?.upsells) ? lead.upsells : []).filter(isLegacyCustomServiceLine);
  return [...customServices, ...legacyCustomServices];
}
...
function sourceIdForLead(lead) {
  ...
  if (s.includes("ads") || s === "ad") return "ads";
...
export async function getCrmKitData({ limit = 200 } = {}) {
  ...
  const uiJobs = leads.map((lead) => {
    ...
    // A lead has real vendor pricing recorded (even if genuinely $0) once
    // vendorQuoteByServiceExVat has explicit per-service keys — distinct from
    // the "never set" state where 0 just means the zeroing bug / no data yet.
    const vendorPricingRecorded = Boolean(
      lead?.vendorQuoteByServiceExVat &&
        typeof lead.vendorQuoteByServiceExVat === "object" &&
        Object.keys(lead.vendorQuoteByServiceExVat).length
    );
    ...
    // Custom service/package lines are separate from true upsells. Replacement
    // custom lines replace the base package total; upsells are always additive.
    const customItems = customServiceLines(lead);
    const replacementCustomItems = customItems.filter((u) => u?.billingMode === "replacement");
    const additiveCustomItems = customItems.filter((u) => u?.billingMode !== "replacement");
    const replacementCustomTotal = replacementCustomItems.reduce((s, u) => s + (safeNum(u?.amountExVat) ?? 0), 0);
    const additiveCustomTotal = additiveCustomItems.reduce((s, u) => s + (safeNum(u?.amountExVat) ?? 0), 0);
    const upsellClientTotal = upsellLines(lead).reduce((s, u) => s + (safeNum(u?.amountExVat) ?? 0), 0);
    const extraClientTotal = additiveCustomTotal + upsellClientTotal;

    // Extra vendor cost — sum from confirmed requests that have a vendorExVat.
    // Avoids depending on requestId being set on the line item (may be missing for
    // older confirmed extras). Use inc VAT — M&C pays Izimoto's VAT on extras too.
    const upsellRequests = Array.isArray(lead?.upsellRequests) ? lead.upsellRequests : [];
    const extraVendorTotal = upsellRequests.reduce((s, r) => {
    ...
    const baseRevenue = replacementCustomTotal > 0 ? 0 : clientQuote ?? vendor ?? 0;
    const revenue = baseRevenue + replacementCustomTotal + extraClientTotal;

    // commission = (base/replacement client - base vendor where applicable) + (extras client - extras vendor)
    // Note: vendor may be 0 for deferred jobs — that's fine, commission = clientQuote - 0 if no vendor cost known
    const baseCommission =
      replacementCustomTotal > 0
        ? Math.max(0, replacementCustomTotal - extraVendorTotal)
        : clientQuote != null
          ? Math.max(0, clientQuote - (vendor ?? 0))
          : 0;
    const extraCommission = replacementCustomTotal > 0 ? Math.max(0, extraClientTotal) : Math.max(0, extraClientTotal - extraVendorTotal);
    ...
    return {
      ...
      invoice,
      booking,
      izimotoCost: (replacementCustomTotal > 0 ? 0 : vendor) + extraVendorTotal,
      vendorCostMissing: vendor === 0 && clientQuote != null && !lead?.vendorPricingDeferred && !vendorPricingRecorded,
      commission,
      revenue,
      value: revenue,
  ...
  // For most services the deposit is a flat 60% of revenue. Paint-correction
  // funnel leads don't follow that model (a flat R1,000 slot hold, then a
  // drop-off/pickup split - see app/paint-correction/pcData.js's
  // pcPaymentSchedule) - collectedPortion() uses their real recorded amounts
  // instead, and falls back to the 60% assumption for everything else (which
  // is provably identical to the old flat calc for non-PC jobs).
  function collectedPortion(j) {
    const pc = j.raw?.funnel === "paint-correction" ? j.raw?.paintCorrection : null;
    if (pc) {
      const hold = safeNum(pc.holdAmount) ?? safeNum(pc.deposit) ?? 0;
      const dropoffPortion = pc.dropoffPaidAt ? (safeNum(pc.dueAtDropoff) ?? safeNum(pc.balance) ?? 0) : 0;
      return hold + dropoffPortion;
    }
    return Math.round((safeNum(j.revenue) ?? 0) * 0.6);
  }
  const depositPaidJobs = uiJobs.filter((j) => String(j.invoice?.status || "") === "deposit_paid");
  const depositRevenue = depositPaidJobs.reduce((s, j) => s + collectedPortion(j), 0);
  const depositProfit = depositPaidJobs.reduce((s, j) => {
    const revenue = safeNum(j.revenue) ?? 0;
    const commission = safeNum(j.commission) ?? 0;
    if (revenue <= 0) return s;
    const fraction = collectedPortion(j) / revenue;
    return s + Math.round(commission * fraction);
  }, 0);
