...
export async function getCrmKitData({ limit = 200 } = {}) {
  ...
  const uiJobs = leads.map((lead) => {
    ...
    // Upsell vendor cost — cross-reference confirmed upsell requests by requestId
    const upsellRequests = Array.isArray(lead?.upsellRequests) ? lead.upsellRequests : [];
    const requestById = Object.fromEntries(upsellRequests.map((r) => [r.id, r]));
    const upsellVendorTotal = upsellItems.reduce((s, u) => {
      const req = u?.requestId ? requestById[u.requestId] : null;
      return s + (safeNum(req?.vendorExVat) ?? 0);
    }, 0);

    const baseRevenue = clientQuote ?? (vendor ? Math.round(vendor * (1 + commissionPercent / 100)) : 0);
    const revenue = baseRevenue + upsellClientTotal;
    // commission = (base client - base vendor) + (upsell client - upsell vendor)
    const baseCommission = clientQuote != null && vendor ? Math.max(0, clientQuote - vendor) : 0;
