...
export async function getCrmKitData({ limit = 200 } = {}) {
  ...
  const uiJobs = leads.map((lead) => {
    ...
    const replacementUpsells = upsellItems.filter((u) => u?.billingMode === "replacement");
    const additiveUpsells = upsellItems.filter((u) => u?.billingMode !== "replacement");
    const replacementClientTotal = replacementUpsells.reduce((s, u) => s + (safeNum(u?.amountExVat) ?? 0), 0);
    const additiveUpsellClientTotal = additiveUpsells.reduce((s, u) => s + (safeNum(u?.amountExVat) ?? 0), 0);
    const upsellClientTotal = replacementClientTotal + additiveUpsellClientTotal;
    ...
    const baseRevenue = replacementClientTotal > 0 ? 0 : clientQuote ?? vendor ?? 0;
    ...
    const baseCommission = replacementClientTotal > 0 ? 0 : clientQuote != null ? Math.max(0, clientQuote - (vendor ?? 0)) : 0;
    ...
    return {
      ...
      invoice,
      booking,
      izimotoCost: (replacementClientTotal > 0 ? 0 : vendor) + upsellVendorTotal,
