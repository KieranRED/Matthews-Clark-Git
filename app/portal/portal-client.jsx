...
export default function PortalClient({ initialClientId, initialToken }) {
  ...
  const [loginState, setLoginState] = useState({ state: "idle", message: "" }); // idle | sending | sent | error
  ...
  const [sheet, setSheet] = useState(null); // add | request
  const [newVehicle, setNewVehicle] = useState({ label: "", plate: "", colour: "", year: "", make: "", model: "" });
  const [request, setRequest] = useState({ vehicleLabel: "", services: { ppf: false, wrap: false, ceramic: false, tint: false, wheel: false, correct: false, detail: false }, notes: "", timeframe: "no-rush" });
  ...
  useEffect(() => {
    // Prefer live link params if present; else restore session.
    if (initialClientId && initialToken) return;
    try {
      const raw = localStorage.getItem("mc_portal_session");
      if (!raw) return;
      const parsed = JSON.parse(raw);
      if (parsed?.c && parsed?.t) {
        setC(String(parsed.c));
        setT(String(parsed.t));
      }
    } catch {
      // ignore
    }
  ...
  async function loadSession({ nextC, nextT } = {}) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/portal/session?c=${encodeURIComponent(clientId)}&t=${encodeURIComponent(token)}`, { cache: "no-store" });
      const json = await res.json().catch(() => null);
      if (!res.ok) throw new Error(json?.error || "Failed to load portal.");
      setData(json);
      setClientMarkedPaid(false);
      try {
        localStorage.setItem("mc_portal_session", JSON.stringify({ c: clientId, t: token }));
      } catch {
        // ignore
      }
      // Clear query params for a cleaner "app" feel.
      try {
      ...
      showToast(msg);
      if (msg.toLowerCase().includes("expired") || msg.toLowerCase().includes("new link")) {
        try {
          localStorage.removeItem("mc_portal_session");
        } catch {
          // ignore
        }
        setC("");
  ...
  async function sendMagicLink() {
    ...
    try {
      const res = await fetch("/api/portal/login", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: e }) });
      const json = await res.json().catch(() => null);
  ...
  return (
    <div className="crm-root cp-shell" id="shell" style={{ width: "min(430px, 100%)", margin: "0 auto" }}>
          ...
          <div className="body">
            <div className="lead">
              ...
              <h1 style={{ marginTop: 14 }}>
                Open <span className="a">/</span> your build.
              </h1>
              <p>Enter the email we have on file. We’ll send you a single-use link — no password.</p>
            </div>
            <div className="field">
              <label htmlFor="cp-email">Email address</label>
              <input id="cp-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" placeholder="you@example.com" />
            ...
            <div className="sent">
              ...
              <div style={{ fontSize: 13, color: "var(--fg-2)", lineHeight: 1.5 }}>The link is valid for 15 minutes. Open it on this device.</div>
            </div>
      ...
      <div className="greeting">
        ...
        <h1>
          Your <span className="acc">build</span>.
        </h1>
      ...
      {activeLead ? (
        <div className="cp-hero">
          ...
          <div className="who">
            <div className="car">
              {String(activeLead.car || "Your vehicle").split(" ")[0] || "Vehicle"} <span className="a">{String(activeLead.car || "").split(" ").slice(1).join(" ")}</span>
            </div>
        ...
        <div className="cp-update" style={{ marginTop: 14 }}>
          ...
          <div className="row">
            ...
            {["deposit_paid", "paid"].includes(String(activeLead.invoiceStatus || "due")) ? (
              <a
                href={bookingUrlForLead(activeLead)}
                className="links"
                style={{ fontFamily: "Archivo, sans-serif", fontWeight: 700, fontSize: 11, letterSpacing: ".18em", color: "var(--mc-blue)", textTransform: "uppercase" }}
              >
                BOOK IN →
              </a>
            ) : null}
      ...
      <div className="section-h" style={{ padding: "32px var(--pad) 14px" }}>
        <div className="section-title">
          YOUR <span className="acc">GARAGE</span>
        </div>
      ...
      <div className="cp-garage">
        {(Array.isArray(client?.vehicles) ? client.vehicles : []).map((v) => {
          ...
          return (
            <div key={v.id} className={"car " + (on ? "active" : "")} onClick={() => setActiveVehicleId(String(v.id))}>
              <div className="thumb" />
              <div className="nm">{v.label}</div>
              <div className="det">{[v.year, v.colour, v.plate].filter(Boolean).join(" · ").toUpperCase()}</div>
              {on ? (
                <button type="button" className="bigbtn bigbtn--p" style={{ marginTop: 8 }} onClick={() => openSheet("request")}>
      ...
      <div className="section-h" style={{ padding: "32px var(--pad) 14px" }}>
        <div className="section-title">
          YOUR <span className="acc">HISTORY</span>
        </div>
            ...
            return (
              <div key={l.id} className="cp-update" style={{ margin: 0 }}>
                ...
                <div className="row">
                  <div className="links">
                    {inv ? (
                      <a href={inv} target="_blank" rel="noreferrer">
                        <span className="a">▸</span> INVOICE
                      </a>
                    ) : (
      ...
      <div className={"cp-toast " + (toast ? "show" : "")} id="toast">
        ▸ {toast || ""}
      </div>
    </div>
  );
