...
export default function LeadsScreen({ index, params }) {
  ...
  const ARCHIVED_STAGES = new Set(["lost", "delivered"]);
  ...
  const filtered = useMemo(() => {
    ...
    const list = jobs.filter((j) => {
      ...
      if (tab === "all") {
        // All active leads — exclude archived stages
        if (ARCHIVED_STAGES.has(j.stage)) return false;
    ...
    <div className="screen">
      ...
      <div className="tabs">
        {tabs.map((t) => {
          ...
          const ct = t.id === "all"
            ? (index?.JOBS || []).filter((j) => !ARCHIVED_STAGES.has(j.stage)).length
            : t.id === "archived"
            ? (index?.JOBS || []).filter((j) => ARCHIVED_STAGES.has(j.stage)).length
            : (index?.JOBS || []).filter((j) => j.stage === stageFilter).length;
          return (
