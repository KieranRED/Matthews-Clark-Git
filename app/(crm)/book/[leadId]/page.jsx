...
export const metadata = { title: "Book in — Matthews & Clark" };
...
export default async function BookingPage({ params, searchParams }) {
    ...
    return (
      <main style={{ minHeight: "100svh", display: "grid", placeItems: "center", background: "#050505", color: "#fff", padding: 24 }}>
        <div style={{ maxWidth: 520, width: "100%", border: "1px solid rgba(255,255,255,.12)", borderRadius: 18, padding: 18 }}>
          ...
          <div style={{ marginTop: 8, opacity: 0.8 }}>Ask the team to resend the booking link.</div>
        </div>
      </main>
    );
  }
  ...
  return <BookingClient leadId={leadId} token={token} lead={lead} />;
}
